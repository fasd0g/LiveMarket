...
import me.livemarket.util.ItemStackSerializer;
...
public class MarketService {
    ...
    /**
     * Шаблон/"витринный" предмет для каждого материала (для отображения и покупки).
     * Позволяет хранить кастомный meta (имя/лоры/чары/CustomModelData).
     */
    private final Map<Material, ItemStack> templates = new ConcurrentHashMap<>();
    ...
    public void loadFromConfigAndDb() {
        ...
        for (String key : sec.getKeys(false)) {
            ...
            // кастомный шаблон предмета (если задан)
            ItemStack tpl = null;
            try {
                String b64 = s.getString("template");
                tpl = ItemStackSerializer.fromBase64(b64);
            } catch (Throwable ignored) {}
            if (tpl != null && tpl.getType() != mat) {
                // на всякий: шаблон должен совпадать по типу
                tpl = null;
            }
            if (tpl == null) tpl = new ItemStack(mat);
            tpl.setAmount(1);
            templates.put(mat, tpl);
    ...
    /**
     * Возвращает шаблон/витринный ItemStack для материала (amount=1).
     * Если шаблона нет, вернёт обычный ItemStack.
     */
    public ItemStack getTemplate(Material mat) {
        ItemStack tpl = templates.get(mat);
        if (tpl == null) return new ItemStack(mat);
        ItemStack c = tpl.clone();
        c.setAmount(1);
        return c;
    }
...
private double clamp(double v, double mn, double mx) {
    ...
    public boolean buy(Player p, MarketItem it, int qty) {
        ...
        // Выдаём по шаблону (с meta), если он задан
        Map<Integer, ItemStack> left = giveByTemplate(p, it.getMaterial(), qty);
        if (!left.isEmpty()) {
    ...
    /**
     * Выдаёт предмет игроку используя шаблон (с meta). Возвращает остаток (как Inventory#addItem).
     */
    private Map<Integer, ItemStack> giveByTemplate(Player p, Material mat, int qty) {
        if (qty <= 0) return java.util.Map.of();
        ItemStack tpl = getTemplate(mat);
        java.util.Map<Integer, ItemStack> leftAll = new java.util.HashMap<>();
        int remaining = qty;
        while (remaining > 0) {
            int give = Math.min(remaining, tpl.getMaxStackSize());
            ItemStack stack = tpl.clone();
            stack.setAmount(give);
            java.util.Map<Integer, ItemStack> left = p.getInventory().addItem(stack);
            if (!left.isEmpty()) {
                // вернём именно то, что не влезло (первый и единственный стек)
                leftAll.putAll(left);
                break;
            }
            remaining -= give;
        }
        // если не всё влезло (remaining>0), то добавим остаток
        if (remaining > 0 && leftAll.isEmpty()) {
            ItemStack rest = tpl.clone();
...
public synchronized boolean addMarketItem(ItemStack template, String category, int slot) {
    if (template == null || template.getType() == Material.AIR) return false;
    ...
    // сохраним шаблон (meta) для отображения/покупки
    ItemStack tpl = template.clone();
    tpl.setAmount(1);
    templates.put(mat, tpl);
    ...
    plugin.getConfig().set(path + ".template", ItemStackSerializer.toBase64(tpl));
    plugin.saveConfig();
...
/** Обновить только шаблон ItemStack (meta) для уже существующего товара. */
public synchronized boolean updateTemplate(Material mat, ItemStack template) {
    MarketItem it = items.get(mat);
    if (it == null) return false;
    if (template == null || template.getType() != mat) return false;
    ItemStack tpl = template.clone();
    tpl.setAmount(1);
    templates.put(mat, tpl);
    plugin.getConfig().set("items." + mat.name() + ".template", ItemStackSerializer.toBase64(tpl));
    plugin.saveConfig();
    return true;
}

/**
 * Обновить параметры товара (base/min/max/stockTarget/slot). Обновляем через пересоздание MarketItem,
 * чтобы не ломать потокобезопасность и т.к. поля в MarketItem были финальными.
 */
public synchronized boolean updateItemConfig(Material mat, Double base, Double min, Double max,
                                            Integer stockTarget, Integer slot) {
    MarketItem old = items.get(mat);
    if (old == null) return false;

    double nBase = base != null ? base : old.getBasePrice();
    double nMin  = min  != null ? min  : old.getMinPrice();
    double nMax  = max  != null ? max  : old.getMaxPrice();
    int nStockTarget = stockTarget != null ? stockTarget : old.getStockTarget();
    int nSlot = slot != null ? slot : old.getSlot();

    // sanity
    if (nMin < 0) nMin = 0;
    if (nMax < nMin) nMax = nMin;
    if (nBase < nMin) nBase = nMin;
    if (nBase > nMax) nBase = nMax;
    if (nStockTarget < 0) nStockTarget = 0;
    if (nSlot < 0) nSlot = 0;

    double price = old.getPrice();
    int stock = old.getStock();
    // clamp price into new min/max
    price = Math.max(nMin, Math.min(nMax, price));

    MarketItem updated = new MarketItem(mat, old.getCategory(), nBase, nMin, nMax, price, stock, nStockTarget, nSlot);
    items.put(mat, updated);

    String path = "items." + mat.name();
    plugin.getConfig().set(path + ".base", nBase);
    plugin.getConfig().set(path + ".min", nMin);
    plugin.getConfig().set(path + ".max", nMax);
    plugin.getConfig().set(path + ".stockTarget", nStockTarget);
    plugin.getConfig().set(path + ".slot", nSlot);
    plugin.saveConfig();

    db.upsertItem(mat.name(), updated.getPrice(), updated.getStock());
    return true;
}
