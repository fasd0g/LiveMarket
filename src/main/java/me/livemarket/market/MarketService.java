...
public class MarketService {
    ...
    private final java.util.Map<java.util.UUID, java.util.Map<org.bukkit.Material, Integer>> buysToday = new java.util.HashMap<>();
    private final java.util.Map<java.util.UUID, Integer> buysTodayTotal = new java.util.HashMap<>();
...
private void runDailyUpdate() {
    ...
    // Сбрасываем дневные лимиты покупок
    buysToday.clear();
    buysTodayTotal.clear();

    // Генерация и анонс "товаров дня"
    generateDailyDeals();
...
private void generateDailyDeals() {
    ...
    for (java.util.Map.Entry<org.bukkit.Material, Double> en : dailyDealBonus.entrySet()) save.put(en.getKey().name(), en.getValue());
...
private void announceDailyDeals() {
    ...
    for (java.util.Map.Entry<org.bukkit.Material, Double> en : dailyDealBonus.entrySet()) {
        MarketItem it = items.get(en.getKey());
        if (it == null) continue;
        String nameRu = NameUtil.ru(en.getKey());
        int pct = (int) Math.round(en.getValue() * 100.0);
        lines.add("§e- " + nameRu + " §7(скидка -" + pct + "%)");
    }
...
private double clamp(double v, double mn, double mx) {
    ...
    public boolean buy(Player p, MarketItem it, int qty) {
        ...
        // Дневной лимит покупок на игрока
        int perItemLimit = plugin.getConfig().getInt("settings.economy.dailyBuyLimitPerPlayerPerItem", 0);
        int totalLimit = plugin.getConfig().getInt("settings.economy.dailyBuyLimitPerPlayerTotal", 0);
        if (perItemLimit > 0) {
            int already = buysToday.getOrDefault(p.getUniqueId(), java.util.Collections.emptyMap()).getOrDefault(it.getMaterial(), 0);
            if (already + qty > perItemLimit) {
                p.sendMessage("§cЛимит покупок на сегодня для этого товара: §f" + already + "§7/§f" + perItemLimit);
                return false;
            }
        }
        if (totalLimit > 0) {
            int alreadyTotal = buysTodayTotal.getOrDefault(p.getUniqueId(), 0);
            if (alreadyTotal + qty > totalLimit) {
                p.sendMessage("§cЛимит покупок на сегодня: §f" + alreadyTotal + "§7/§f" + totalLimit);
                return false;
            }
        }
        ...
        net.milkbowl.vault.economy.EconomyResponse res = economy.withdrawPlayer(p, cost);
        ...
        synchronized (it) {
            ...
            buysToday.computeIfAbsent(p.getUniqueId(), k -> new java.util.HashMap<>())
                    .merge(it.getMaterial(), qty, Integer::sum);
            buysTodayTotal.merge(p.getUniqueId(), qty, Integer::sum);
            if (instantAdjustEnabled) updatePriceWithStep(it, instantMaxStepPerTrade);
        }

        p.sendMessage("§aКуплено: §f" + qty + " §f" + NameUtil.ru(it.getMaterial()) + " §7за §f" + format(cost));
        return true;
    }
    ...
    public boolean sell(Player p, MarketItem it, int qty) {
        ...
        p.sendMessage("§aПродано: §f" + rr.removed + " §f" + NameUtil.ru(it.getMaterial())
...
// Загружаем сохранённые "товары дня" или генерируем новые, если пусто
dailyDealBonus.clear();
java.util.Map<String, Double> savedDeals = db.loadDailyDeals();
for (java.util.Map.Entry<String, Double> en : savedDeals.entrySet()) {
    try {
        org.bukkit.Material m = org.bukkit.Material.valueOf(en.getKey());
        if (items.containsKey(m)) dailyDealBonus.put(m, en.getValue());
    } catch (Exception ignored) {}
}
if (dailyDealBonus.isEmpty()) {
    generateDailyDeals();
}
