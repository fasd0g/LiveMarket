...
public class GenerateCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length >= 1) {
            try {
                perCat = Math.max(1, Math.min(45, Integer.parseInt(args[0])));
        ...
        for (String cat : byCat.keySet()) {
            ...
            int take = Math.min(perCat, mats.size());
            int slotIdx = 0;
        ...
        try {
            ...
            sender.sendMessage("§7Параметр: /lmgen [countPerCategory], сейчас: §f" + perCat);
        } catch (Exception e) {
